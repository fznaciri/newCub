...
void init_wall(int i)
{
    ...
    g_wall.h = (TILE_SIZE / g_wall.dist) * ((g_game.win_w / 2) / (tan(FOV / 2)));
...
void wall(int i)
{
    ...
    while (g_wall.j < g_wall.bot) // wall 
    {
        dist_from_top = g_wall.j + (g_wall.h / 2) - (g_game.win_h / 2);
                
        g_tex[g_wall.t].pos.y = dist_from_top * ((float)g_tex[g_wall.t].h / g_wall.h);
        g_tex[g_wall.t].pos.y = g_tex[g_wall.t].pos.y >= g_game.win_h ? g_game.win_h - 1 : g_tex[g_wall.t].pos.y;
        g_wall.color = get_text_color(g_tex[g_wall.t], g_tex[g_wall.t].pos.x, g_tex[g_wall.t].pos.y);
        my_mlx_pixel_put(i, g_wall.j, g_wall.color);
        g_wall.j++;
    } 
}
...
void render_3dwall()
{
    ...
    while(i < g_game.win_w)
    {
        ...
        g_tex[g_wall.t].pos.x = g_tex[g_wall.t].pos.x < 0 ? 0 : g_tex[g_wall.t].pos.x;
        ceeling(i);
        g_wall.j = g_wall.top;
        wall(i);
        g_wall.j = g_wall.bot;
